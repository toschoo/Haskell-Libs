...
instance Render LaTeX where
 ...
 render (TeXMathX e m _) = 
   let (a,z) = case e of
                 MathEnv     -> ("\\begin{math}", "\\end{math}")
                 DispEnv     -> ("\\begin{displaymath}", "\\end{displaymath}")
                 EqEnv       -> ("\\begin{equation}", "\\end{equation}")
                 Parentheses -> ("\\(", "\\)")
                 Square      -> ("\\[", "\\]")
                 Dollar      -> ("$", "$")
    in a <> render m <> z
 render (TeXNewLine b) = " \\\\" <> ( if b then "*" else mempty ) <> " " -- to be removed
 render (TeXLineBreak n u b) = 
   let nu = case n of
              Nothing -> ""
...
instance Render MathType where
   render _ = ""
