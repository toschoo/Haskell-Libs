...
where 
  ...
  -------------------------------------------------------------------------
  conduitParser :: (MonadResource m, Monoid a) => 
                   Parser a -> (Text -> Either String a) -> Conduit Text m a
  conduitParser p final = CU.conduitState Nothing push close
